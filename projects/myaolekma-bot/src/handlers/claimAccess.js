...
async function handleMessage(bot, msg, userStates, config) {
  ...
  if (state.step === 'org_name') {
    ...
    await bot.sendMessage(chatId, 
      `✅ Название: *${text}*\n\n` +
      `Вы *владелец* или *представитель* компании?\n` +
      `Хотите получить права на редактирование?\n\n` +
      `Напишите, как вы связаны с организацией, ` +
      `или отправьте фото с объявлением об изменении:`,
      { parse_mode: 'Markdown' }
    );
    return;
  }
